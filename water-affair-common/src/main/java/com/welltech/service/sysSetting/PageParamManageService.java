			...
			*/
			...
			if(null!=datas && datas.size()>0){
				...
				if(null != heiChouData){
					...
					String heichou = getHeichouLevel(heiChouData);
					result.put("heichou", heichou);
				}
				if(null != diBiaoData){
					datas = new ArrayList<WtParamDataDto>();
					String dibiao = getDibiaoLevel(diBiaoData);
					String dibiao1 = getDibiaoLevel(diBiaoData1);
					dibiao=compareLevel(dibiao, dibiao1);
					//根据最大值和最小值的比较得出一个最差评价然后赋值
					WtParamDataDto mergeDibiao=compareLevel(diBiaoData,diBiaoData1);
				...
				if(StringUtils.isNoneBlank(pollutant)){
					result.put("pollutant", getParamNameByParam(station));
				}
			}
			map.put("paramColumn", avgParamColumn);
			WtParamDataDto data = pageParamManageDao.getDailyReportData(map);
...
//					}
	 ...
	 * @return
	 */
	private String getDibiaoLevel(WtParamDataDto diBiaoData) {
		String level1 = "Ⅰ类";
		String level2 = "Ⅱ类";
		String level3 = "Ⅲ类";
		String level4 = "Ⅳ类";
	 ...
	 * @return
	 */
	private String getHeichouLevel(WtParamDataDto heiChouData) {
		String level1 = "合格";
		String level2 = "轻度黑臭";
		String level3 = "重度黑臭";
		String result = "";
	...
	public String getParamNameByParam(WtStation station){
